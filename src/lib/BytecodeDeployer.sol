...
contract BytecodeDeployer is Test {
    ...
    function deployContract(string memory fileName) public returns (address) {
        string memory bashCommand =
            string.concat('cast abi-encode "f(bytes)" $(cat bytecode/', string.concat(fileName, ".hex | tail -n 1)"));
        ...
        require(deployedAddress != address(0), "BytecodeDeployer could not deploy contract");
